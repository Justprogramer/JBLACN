...
def get_ner_fmeasure(golden_lists, predict_lists, word_dic, label_type="BMES", name='joint', need_save_matrix=False):
    right_tag = 0
    all_tag = 0
    golden_labels = [word_dic.get_instance(ix) for ix in golden_lists]
    predict_labels = [word_dic.get_instance(ix) for ix in predict_lists]
    for idy in range(len(predict_labels)):
        if golden_labels[idy] == predict_labels[idy]:
            right_tag += 1
    all_tag += len(golden_labels)
    if label_type == "BMES":
        gold_matrix = get_ner_BMES(golden_labels)
        pred_matrix = get_ner_BMES(predict_labels)
    else:
        gold_matrix = get_ner_BIO(golden_labels)
        pred_matrix = get_ner_BIO(predict_labels)
    # print "gold", gold_matrix
    # print "pred", pred_matrix
    if need_save_matrix:
        with codecs.open("%s_list_result.json" % name, 'w', 'utf-8') as w:
            list_dict = {"gold": gold_matrix, "pred": pred_matrix}
            json.dump(list_dict, w)
