...
class multihead_attention(nn.Module):
    ...
    def __init__(self, num_units, num_heads=1, dropout_rate=0, gpu=True, causality=False):
        ...
        self.Q_proj = nn.Sequential(nn.Linear(self.num_units, self.num_units), nn.ReLU(inplace=True))
        self.K_proj = nn.Sequential(nn.Linear(self.num_units, self.num_units), nn.ReLU(inplace=True))
        self.V_proj = nn.Sequential(nn.Linear(self.num_units, self.num_units), nn.ReLU(inplace=True))
        if self.gpu:
    ...
    def forward(self, queries, keys, values, last_layer=False):
        ...
        # if outputs.size(1) > 1 and last_layer == False:
        #     with codecs.open("self-attention.txt", "a", "utf-8") as w:
        #         obj = outputs.data.cpu().numpy().tolist()
        #         w.write("%s\n" % obj)
        # Dropouts
